"""
Live stair detection with RealSense camera.
...
class LiveStairDetector:
    """Real-time stair detection using RealSense camera."""
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # Show image
                cv2.imshow('Live Stair Detection', display_image)
                ...
                if key == ord('q'):
                    ...
                    filename = f"stair_detection_{frame_count}.jpg"
...
def main():
    ...
    detector = LiveStairDetector(str(config_path))
    detector.run()
